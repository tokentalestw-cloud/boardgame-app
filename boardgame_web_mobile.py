...
DB_PATH = BASE_DIR / "boardgame_records.db"
GAME_IMAGE_DIR = BASE_DIR / "game_images"
MEMBER_IMAGE_DIR = BASE_DIR / "member_images"
GAME_IMAGE_DIR.mkdir(exist_ok=True)
MEMBER_IMAGE_DIR.mkdir(exist_ok=True)

APP_TITLE = "桌遊遊玩紀錄系統｜手機網頁版"
...
def create_tables() -> None:
    ...
    if "bgg_score" not in game_columns:
        cur.execute("ALTER TABLE games ADD COLUMN bgg_score REAL")
    if "user_rating" not in game_columns:
        cur.execute("ALTER TABLE games ADD COLUMN user_rating REAL")
    if "intro" not in game_columns:
        cur.execute("ALTER TABLE games ADD COLUMN intro TEXT DEFAULT ''")
    if "notes" not in game_columns:
        cur.execute("ALTER TABLE games ADD COLUMN notes TEXT DEFAULT ''")
    if "last_play_date" not in game_columns:
        cur.execute("ALTER TABLE games ADD COLUMN last_play_date TEXT DEFAULT ''")
    if "play_count" not in game_columns:
        cur.execute("ALTER TABLE games ADD COLUMN play_count INTEGER DEFAULT 0")

    cur.execute("PRAGMA table_info(play_records)")
    record_columns = [row[1] for row in cur.fetchall()]
    if "game_id" not in record_columns:
        cur.execute("ALTER TABLE play_records ADD COLUMN game_id INTEGER")
...
def web_image_path(db_path: str) -> str:
    ...
    # fallback for already-stored relative strings
...
def sync_game_stats() -> None:
    ...
    for row in cur.fetchall():
        cur.execute(
            "UPDATE games SET play_count = ?, last_play_date = ? WHERE name = ?",
            (row["play_count"], row["last_play_date"], row["game_name"]),
        )
    conn.commit()
    conn.close()


def get_member_best_type(conn: sqlite3.Connection, member_name: str) -> tuple[Optional[str], Optional[str]]:
    cur = conn.cursor()
...
def get_top_player_by_game_type(conn: sqlite3.Connection) -> list[dict]:
    cur = conn.cursor()
    cur.execute("SELECT game_type, players, winners FROM play_records")
...
def summary_stats(conn: sqlite3.Connection) -> dict:
    ...
    cur.execute("SELECT COUNT(*) FROM play_records")
    records = cur.fetchone()[0]
    cur.execute("SELECT COALESCE(MAX(play_date), '') FROM play_records")
    latest = cur.fetchone()[0] or "尚無紀錄"
...
def page_template(title: str, body: str, active: str = "dashboard", notice: str = "") -> HTMLResponse:
    ...
    <head>
      ...
      <style>
        :root {{
          --bg:#f5f7fb; --card:#fff; --text:#243447; --sub:#5b6b7a; --line:#dde6f0; --accent:#5b8ff9;
          --soft:#eef3fa; --warn:#fff7e6; --warn-line:#f6c667;
        }}
...
def dashboard(request: Request, notice: str = ""):
    ...
    hero_cards = "".join(
        f'''<div class="hero-card">
              <img class="hero-avatar" src="{escape(h['image_path'])}" {'style="display:none"' if not h['image_path'] else ''}>
              <div>
                <div><strong>👑 {escape(h['game_type'])}</strong></div>
                <div>{escape(h['player'])}</div>
                <div class="small">勝率 {h['win_rate']:.1f}%｜勝場 {h['wins']} / {h['games']}</div>
              </div>
            </div>'''
        for h in heroes
...
def games_page(request: Request, notice: str = ""):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, name, category, image_path, bgg_score, user_rating, COALESCE(last_play_date,'') AS last_play_date, COALESCE(play_count,0) AS play_count FROM games ORDER BY name COLLATE NOCASE"
    )
    rows = cur.fetchall()
    conn.close()
    ...
    for game in rows:
        ...
        items.append(
            f'''<div class="item">
                <img class="thumb" src="{escape(image_url)}" {'style="display:none"' if not image_url else ''}>
                <div>
                  <h3>{escape(game['name'])}</h3>
                  <div class="meta">類型：{escape(game['category'])}<br>BGG：{game['bgg_score'] if game['bgg_score'] is not None else '未填寫'}<br>玩家評分：{game['user_rating'] if game['user_rating'] is not None else '未填寫'}<br>遊玩次數：{game['play_count']}<br>最後遊玩：{escape(game['last_play_date'] or '尚無紀錄')}</div>
                </div>
                <div class="spacer"></div>
                <form method="post" action="/games/{game['id']}/delete" onsubmit="return confirm('確定要刪除這款桌遊嗎？');">
...
async def add_game(
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO games (name, category, image_path, bgg_score, user_rating) VALUES (?, ?, ?, ?, ?)",
            (name.strip(), category.strip(), image_path, bgg_value, rating_value),
        )
        conn.commit()
...
def members_page(request: Request, notice: str = ""):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, name, favorite_types, image_path FROM members ORDER BY name COLLATE NOCASE")
    rows = cur.fetchall()
    ...
    for member in rows:
        ...
        cards.append(
            f'''<div class="item">
                <img class="thumb circle" src="{escape(image_url)}" {'style="display:none"' if not image_url else ''}>
                <div>
                  <h3>{escape(member['name'])}</h3>
                  <div class="meta">擅長：{escape(member['favorite_types'] or '未填寫')}<br>總場數：{games_played}｜勝場：{wins}｜勝率：{rate:.1f}%<br>近期遊玩：{escape('、'.join(recent_games) if recent_games else '尚無紀錄')}</div>
                  {best_line}
                </div>
                <div class="spacer"></div>
                <form method="post" action="/members/{member['id']}/delete" onsubmit="return confirm('確定要刪除這位成員嗎？');">
                  <button class="danger" type="submit">刪除</button>
                </form>
              </div>'''
        )
...
async def add_member(
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO members (name, favorite_types, image_path) VALUES (?, ?, ?)",
            (name.strip(), favorite_types.strip(), image_path),
        )
        conn.commit()
...
def records_page(request: Request, notice: str = ""):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT name, category FROM games ORDER BY name COLLATE NOCASE")
    games = cur.fetchall()
    cur.execute("SELECT name FROM members ORDER BY name COLLATE NOCASE")
    ...
    game_options = "".join(f'<option value="{escape(g["name"])}" data-type="{escape(g["category"])}">{escape(g["name"])}｜{escape(g["category"])} </option>' for g in games)
    member_options = "".join(f'<option value="{escape(m["name"])}">' for m in members)
    record_rows = "".join(
        f'''<tr>
          <td>{escape(r['play_date'])}</td>
          <td>{escape(r['game_name'])}</td>
          <td>{escape(r['game_type'])}</td>
          <td>{escape(r['players'])}</td>
          <td>{escape(r['winners'])}</td>
          <td>
            <form method="post" action="/records/{r['id']}/delete" onsubmit="return confirm('確定要刪除這筆紀錄嗎？');">
              <button class="danger" type="submit">刪除</button>
            </form>
          </td>
        </tr>'''
    ...
    body = f"""
    <div class="card">
      <div class="card-title">新增遊玩紀錄</div>
      <form class="form-grid two" method="post" action="/records">
        ...
        <div>
          <label>桌遊</label>
          <input name="game_name" list="games_list" required>
          <datalist id="games_list">{game_options}</datalist>
        </div>
      ...
      <div class="small" style="margin-top:8px;">桌遊類型若留空，系統會自動從桌遊資料庫帶入。</div>
    </div>
    <div class="card">
      <div class="card-title">遊玩紀錄</div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>日期</th><th>桌遊</th><th>類型</th><th>玩家</th><th>勝者</th><th></th></tr></thead>
          <tbody>{record_rows or '<tr><td colspan="6">目前尚無紀錄</td></tr>'}</tbody>
        </table>
      </div>
    </div>
    """
...
def add_record(
    ...
    if not game_type.strip():
        cur.execute("SELECT id, category FROM games WHERE name = ?", (game_name,))
    else:
        cur.execute("SELECT id, category FROM games WHERE name = ?", (game_name,))
    ...
    cur.execute(
        "INSERT INTO play_records (play_date, game_id, game_name, game_type, players, winners) VALUES (?, ?, ?, ?, ?, ?)",
        (play_date, game_id, game_name, category, ", ".join(player_list), ", ".join(winner_list)),
    )
    conn.commit()
    conn.close()
    sync_game_stats()
...
def delete_record(record_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM play_records WHERE id = ?", (record_id,))
    conn.commit()
    conn.close()
    sync_game_stats()
...
if __name__ == "__main__":
    ...
    uvicorn.run("boardgame_web_mobile:app", host="0.0.0.0", port=port, reload=False)
